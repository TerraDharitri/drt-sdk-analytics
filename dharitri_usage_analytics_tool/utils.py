...
from dharitri_usage_analytics_tool.constants import (
    BLUE_REPORT_PORT,
    DATE_FORMAT,
    DAYS_IN_MONTHLY_REPORT,
    DAYS_IN_TWO_WEEKS_REPORT,
    DAYS_IN_WEEK,
    GREEN_REPORT_PORT,
...
class PackagesRegistries(Enum):
    NPM = PackagesRegistry(
        "npmjs", "https://registry.npmjs.org/-/v1/search", "https://api.npmjs.org/downloads/range", [Reports.BLUE.value]
    )
    CARGO = PackagesRegistry(
        "crates.io", "https://crates.io/api/v1/crates", "https://crates.io/api/v1/crates", [Reports.BLUE.value]
    )
    PYPI = PackagesRegistry(
        "pypi", "https://pypi.org/search", "https://pypistats.org/api/packages", [Reports.BLUE.value]
    )
    GITHUB = PackagesRegistry(
        "github", "https://api.github.com/search/repositories", "https://api.github.com/repos", [Reports.GREEN.value]
    )
...
class UserAgentGroups(Enum):
    ...
    def find(user_agent_name: str) -> str:
        ...
        if group in [UserAgentGroups.DHARITRI.value, UserAgentGroups.UNKNOWN.value]:
            return user_agent_name
        elif group in [
            UserAgentGroups.AXIOS.value,
            UserAgentGroups.PYTHON.value,
            UserAgentGroups.APACHE.value,
            UserAgentGroups.OKHTTP.value,
            UserAgentGroups.CURL.value,
        ]:
    ...
    def get_group(user_agent_name: str) -> UserAgentGroup:
        group = next(
            (
                group
                for group in [item.value for item in UserAgentGroups]
                if any(
                    re.search(UserAgentGroups._safe_pattern(pattern), user_agent_name, re.IGNORECASE)
                    for pattern in group.group_prefixes
                )
            ),
...
async def select_report(page: Page, selected_file: str) -> str:
    ...
    selected_value: str = await page.evaluate(
        f"""
    ...
    output = (
        f"{file_name}.pdf"
        if any(repo_type in selected_value for repo_type in ["blue", "green", "yellow"])
        else "combined.pdf"
    )
