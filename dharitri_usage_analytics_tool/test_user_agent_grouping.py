from dharitri_usage_analytics_tool.utils import UserAgentGroups
...
class TestUserAgentGroups:
    ...
    def test_dharitri_grouping(self):
        key = "Dharitri/1.0.1 (GO SDK tools)"
        ...
        key = "dharitri-sdk/proxy/unknown"
        assert UserAgentGroups.get_group(key) == UserAgentGroups.DHARITRI.value
        assert UserAgentGroups.find(key) == key

        key = "dharitri-sdk/proxy/mvx-stay-guarded"
        assert UserAgentGroups.get_group(key) == UserAgentGroups.DHARITRI.value
        assert UserAgentGroups.find(key) == key
