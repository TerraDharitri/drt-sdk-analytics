...
from dharitri_usage_analytics_tool.package_managers_fetcher import PackageManagersFetcher, PackageManagersPackage
...
def get_layout():
    ...
    selected_option = (
        dropdown_options[0]["value"] if dropdown_options else None
    )  # Set default value as the newest file generated
    organization_options = [item.value.name for item in EcosystemConfiguration]

    # Layout of the Dash app
    return html.Div(
        style={"backgroundColor": report_type.repo_color},
        children=[
            html.Div(
                style={"display": "flex", "alignItems": "center"},
                children=[
                    html.H1(report_type.repo_title, style={"marginRight": "20px", "width": "30%"}),
                    dcc.Dropdown(
                        id="file-selector",
                        maxHeight=1000,
...
def create_table(fetcher: PackageManagersFetcher, section: PackagesRegistry):
    ...
    for package in packages:
        package_statistics = package.create_summary_statistics_from_daily_downloads(
            fetcher.end_date, report_type.repo_length
        )
        row = [
            ...
            html.Td(
                package_statistics["libraries_io_score"], id=f"lio_{package.package_name}", style={"textAlign": "right"}
            ),
            html.Td(package_statistics["site_score"], style={"textAlign": "right", "width": "100px"}),
            html.Td(
                " - " + package_statistics["site_score_details"],
                style={
                    "textAlign": "right",
                },
            ),
...
def create_package_info_box(fetcher: PackageManagersFetcher, section: PackagesRegistry):
    info_boxes = []
    packages: list[PackageManagersPackage] = [
        cast(PackageManagersPackage, item) for item in fetcher.packages if item.package_site == section.repo_name
    ]
    packages.sort(key=lambda pkg: pkg.no_of_downloads, reverse=True)
...
@app.callback(
    Output("report-content", "children"), [Input("file-selector", "value"), Input("organization-selector", "value")]
)
            ...
            dcc.Tabs(
                [
                    dcc.Tab(
                        ...
                        id=repo.value.repo_name.replace(".", "-"),
                        ...
                        children=[
                            ...
                            html.Div(
                                [
                                    dcc.Graph(id="downloads-graph", figure=create_graph(fetcher, repo.value)),
                                ],
                    ...
                    if report_type in repo.value.reports
                ],
                colors={
