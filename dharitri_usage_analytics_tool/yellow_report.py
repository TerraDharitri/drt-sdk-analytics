...
from dharitri_usage_analytics_tool.elastic_fetcher import ElasticSearchFetcher, ElasticSearchPackage
from dharitri_usage_analytics_tool.fetcher import Package
from dharitri_usage_analytics_tool.utils import FormattedDate, Reports, get_environment_var
...
def create_table(fetcher: ElasticSearchFetcher, section: str):
    ...
    row = html.Tr(
        [
            ...
            html.Td(
                f'{total["total_usage"]:,}', style={"textAlign": "right", "maxWidth": "10ch", "fontWeight": "bold"}
            ),
            html.Td(f'{total["last_week_usage"]:,}', style={"textAlign": "right", "fontWeight": "bold"}),
            html.Td(
                f'{total["total_usage"] / report_type.repo_length:,.0f}',
...
def create_graph(fetcher: ElasticSearchFetcher, section: str) -> Dict[str, Any]:
    ...
    return {
        "data": traces,
        "layout": go.Layout(
            title="Daily Usage Evolution", xaxis={"title": "Date"}, yaxis={"title": "Usage"}, hovermode="closest"
        ),
    }


@app.callback(Output("report-content", "children"), Input("file-selector", "value"))
            ...
            dcc.Tabs(
                [
                    dcc.Tab(
                        ...
                        children=[
                            html.H1(
                                f"{organization.name} - {section.replace('_', ' ')} - API User Agent Access Details"
                            ),
                            ...
                            html.Div(
                                [
                                    dcc.Graph(id="downloads-graph", figure=create_graph(fetcher, section)),
                                ],
