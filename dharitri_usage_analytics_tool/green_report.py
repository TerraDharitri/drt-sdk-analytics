...
from dharitri_usage_analytics_tool.github_fetcher import GithubFetcher, GithubPackage
    ...
    return html.Div(
        ...
        children=[
            html.Div(
                ...
                children=[
                    html.H1(report_type.repo_title, style={"marginRight": "20px", "width": "15%"}),
                    dcc.Dropdown(
                        id="file-selector",
                        maxHeight=1000,
...
def create_downloads_graph(fetcher: GithubFetcher, section: PackagesRegistry, language: str) -> Dict[str, Any]:
    ...
    return {
        "data": traces,
        "layout": go.Layout(
            title="Daily Clones Evolution", xaxis={"title": "Date"}, yaxis={"title": "Clones"}, hovermode="closest"
        ),
    }
...
def create_visits_graph(fetcher: GithubFetcher, section: PackagesRegistry, language: str) -> Dict[str, Any]:
    ...
    return {
        "data": traces,
        "layout": go.Layout(
            title="Daily Visits Evolution", xaxis={"title": "Date"}, yaxis={"title": "Visits"}, hovermode="closest"
        ),
    }


@app.callback(
    Output("report-content", "children"), [Input("file-selector", "value"), Input("language-filter", "value")]
)
            ...
            dcc.Tabs(
                ...
                children=[
                    dcc.Tab(
                        ...
                        children=[
                            html.H1(
                                f"{org.value.name} - {repo.name} Repositories Downloads {'' if selected_language == 'All' else ' - ' + selected_language}"
                            ),
                            ...
                            (
                                create_package_info_box(fetchers[org], repo.value, selected_language)
                                if org.value.report_warnings
                                else None
                            ),
                        ],
                    )
