...
from dharitri_usage_analytics_tool.fetcher import DailyActivity, Fetcher, Package, Score
...
class GithubDailyActivity(DailyActivity):
    ...
    def from_generated_file(cls, response: Dict[str, Any]) -> "GithubDailyActivity":
        ...
        return cls(result.date, result.downloads, uniques)
...
class GithubPackage(Package):
    ...
    def create_summary_statistics_from_daily_downloads(
        self, end_date: str, report_duration=DAYS_IN_TWO_WEEKS_REPORT
    ) -> Dict[str, Any]:
    ...
    def analyse_package(self) -> str:
        main_negatives = ", ".join(
            f"{key} = 0" for key, value in self.main_page_statistics.items() if value == 0 and "has" in key
        )
        score_negatives = ", ".join(
            f"{key} = {value}" for key, value in self.site_score.details.items() if "has" in key and int(value) == 0
        )
...
class GithubFetcher(Fetcher):
    ...
    def _get_github_authorization_header(self) -> Dict[str, Any]:
        ...
        return {"Authorization": f"Bearer {bearer_token}", "Accept": "application/vnd.github.v3+json"}
    ...
    def get_github_package_names(self) -> Dict[str, Any]:  # github api - query search result
        ...
        while True:
            ...
            scores_dict.update(
                {item.get("full_name"): self.build_package_main_page_score(item) for item in package_info}
            )
    ...
    def github_package_language(self, package_name: str, language: str) -> Language:
        packet_language = next(
            (lang.value for lang in Languages if any("-" + suffix in package_name for suffix in lang.value.suffixes)),
            None,
        )
    ...
    def from_package_sites(organization: Organization, end_date: str) -> "GithubFetcher":
        ...
        with tqdm(total=len(packages)) as pbar:
            for package_name in packages.keys():
                fetched_downloads = (
                    result.fetch_github_downloads(package_name) if organization == my_organization else {}
                )
                ...
                if not package_downloads.main_page_statistics["is_forked"]:
                    package_downloads.site_score = Score.from_dict(
                        result.fetch_github_package_community_score(package_name)
                    )
