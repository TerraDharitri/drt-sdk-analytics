...
from dharitri_usage_analytics_tool.constants import DEFAULT_DATE
from dharitri_usage_analytics_tool.ecosystem import Organization
from dharitri_usage_analytics_tool.fetcher import DailyActivity, Fetcher, Package
from dharitri_usage_analytics_tool.indexer import Indexer
...
class ElasticSearchFetcher(Fetcher):
    ...
    def fetch_data(self, indexer: Indexer, index: Index, end_timestamp: FormattedDate) -> ObjectApiResponse[Any]:
        ...
        print(
            f"fetching from {self.organization.name} {index.index_title} ({start_timestamp} - {end_timestamp}) = {count} documents..."
        )
