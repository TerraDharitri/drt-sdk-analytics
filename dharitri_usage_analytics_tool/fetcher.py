...
from dharitri_usage_analytics_tool.constants import DAYS_IN_WEEK, DEFAULT_DATE
...
class Score:
    ...
    def __repr__(self) -> str:
        return ", ".join(
            f"{key} = {float(value):.2f}" if isinstance(value, (float, int)) else f"{key} = {value}"
            for key, value in self.details.items()
        )

    def to_dict(self) -> Dict[str, Any]:
        return {"final": self.final, "detail": self.details}
...
class DailyActivity:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"date": self.date, "downloads": self.downloads}
...
class Package:
    ...
    def calculate_activity_statistics(
        self, name: str, activity: Sequence[DailyActivity], end_date: str, report_duration: int
    ) -> Dict[str, Any]:
...
class Fetcher:
    ...
    def from_generated_file(cls, file_name: str, organization: Organization):
        ...
        if not organization_data and EcosystemConfiguration.DHARITRI.value.name not in json_data.keys():
            organization_data = json_data if organization.name == EcosystemConfiguration.DHARITRI.value.name else {}
