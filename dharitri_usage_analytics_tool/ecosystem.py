...
from dharitri_usage_analytics_tool.constants import CRATES_PAGE_SIZE, GITHUB_PAGE_SIZE, NPM_PAGE_SIZE
from dharitri_usage_analytics_tool.utils import PackagesRegistries, PackagesRegistry
...
class Organization:
    ...
    def get_search_url_string(self, site: PackagesRegistry, page: int) -> str:
        ...
        if site == PackagesRegistries.GITHUB.value:
            ...
            affiliated_string = (
                "+".join(f"user:{item}" for item in self.affiliated_orgs) + "+fork:true+"
                if self.affiliated_orgs
                else ""
            )
    ...
    def get_search_filter(self, site: PackagesRegistry, item: Dict[str, Any]) -> bool:
        ...
        if site == PackagesRegistries.NPM.value:
            ...
            return pattern in item_name and (
                True
                if "@" in item_name
                else (
                    self.name.lower() in item_homepage
                    or isinstance(item_repository, str)
                    and any(own_repo_str in item_repository for own_repo_str in own_github_orgs)
                )
            )
            ...
            return (
                ...
                and isinstance(item_repository, str)
                and any(own_repo_str in item_repository for own_repo_str in own_github_orgs)
            )
            ...
            return any(own_str in source_url for own_str in own_github_orgs) or any(
                own_str in homepage_url for own_str in own_github_orgs
            )
        return False
