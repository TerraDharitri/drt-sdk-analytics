...
class PackageManagersPackage(Package):
    ...
    def analyse_libraries_io_score(self) -> str:
        negatives = ", ".join(
            f"{key} = {value}"
            for key, value in self.libraries_io_score.items()
            if value < 0 or value == 0 and "present" in key
        )
        return negatives

    def create_summary_statistics_from_daily_downloads(
        self, end_date: str, report_duration: int = DAYS_IN_MONTHLY_REPORT
    ) -> Dict[str, Any]:
    ...
    def from_pypi_fetched_data(package: str, lang: str, response: Dict[str, Any]) -> "PackageManagersPackage":
        ...
        result.downloads = [
            PackageManagersDailyActivity.from_pypi_fetched_data(item)
            for item in filter(lambda x: x.get("category", "") == "with_mirrors", raw_downloads)
        ]
        result.package_language = lang
        result.package_name = response.get("package", package)
...
class PackageManagersFetcher(Fetcher):
    ...
    def get_npm_package_names(self) -> Dict[str, Any]:  # npm api (registry.npmjs.org) - query search result
        ...
        while True:
            ...
            scores_dict.update(
                {
                    item.get("package", {}).get("name"): item.get("score", {})
    ...
    def get_crates_package_names(self) -> List[str]:  # crates api (crates/api) - query search result
        ...
        while search_string:
            ...
            new_package_names = [
                item.get("name")
                for item in package_info
                if self.organization.get_search_filter(PackagesRegistries.CARGO.value, item)
            ]
    ...
    def fetch_crates_downloads(self, package_name: str):
        ...
        data["meta"]["extra_downloads"] = [
            entry for entry in data["meta"]["extra_downloads"] if self.start_date <= entry["date"] <= self.end_date
        ]
        return data
    ...
    def fetch_pypi_downloads(self, package_name: str):
        ...
        response.raise_for_status()
        data = response.json()
        data["data"] = [entry for entry in data["data"] if self.start_date <= entry["date"] <= self.end_date]
        return data
    ...
    def from_package_sites(org: Organization, end_date: str) -> "PackageManagersFetcher":
        ...
        with tqdm(total=len(packages)) as pbar:
            for package_name in packages.keys():
                ...
                package_downloads.libraries_io_score = result.fetch_libraries_io_score(
                    package_name, PackagesRegistries.NPM.name
                )
        ...
        with tqdm(total=len(packages)) as pbar:
            for package_name in packages:
                ...
                package_downloads.libraries_io_score = result.fetch_libraries_io_score(
                    package_name, PackagesRegistries.CARGO.name
                )
                result.packages.append(package_downloads)
                pbar.update(1)
        ...
        with tqdm(total=len(packages)) as pbar:
            for package_name in packages:
                ...
                package_downloads.libraries_io_score = result.fetch_libraries_io_score(
                    package_name, PackagesRegistries.PYPI.name
                )
